#############################
my_univ_id = "2011147079"     ## Univ. Num.
...
class QLearningAgent(ReinforcementAgent):
    ...
    def getQValue(self, state, action):
        ...
        if (state == None):
        	return 0.0
        elif(self.qValues[state, action] == None):
        	return 0.0
        else : 
    ...
    def computeValueFromQValues(self, state):
        ...
        for legalaction in self.getLegalActions(state):
        	if(val < self.getQValue(state, legalaction)):
        		val = self.getQValue(state, legalaction)
    ...
    def update(self, state, action, nextState, reward):
        ...
        val = reward + (self.discount * self.getValue(nextState))
        self.qValues[state, action] = Qval + (self.alpha * (val-Qval))
